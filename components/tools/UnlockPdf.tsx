...
export default function UnlockPdf() {
  ...
  const processFile = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Invalid password or processing failed.");
    } finally {
      setProcessing(false);
    }
  };
    ...
    <div className="rounded-2xl border border-border bg-surface p-3 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_-12px_rgba(0,0,0,0.12)] sm:p-4">
      {!file ? (
        ...
        >
          ...
          <p className="text-[15px] font-medium text-ink">Drop your protected PDF here</p>
          <button
            type="button"
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-border p-4 space-y-4">
            <div>
               <label className="block text-sm font-medium text-ink mb-1">Enter Password</label>
               <input 
                  type="password" 
                  value={password}
          ...
          {done && (
            <div className="flex items-center gap-3 rounded-lg bg-ok-soft px-3 py-2 text-ok">
              <p className="text-xs font-medium">Unlocked PDF downloaded successfully!</p>
            </div>
          )}
