...
/** Parse a page-range string like "1,3,5-8" into zero-based indices. */
  ...
  for (const part of parts) {
    ...
    if (rangeMatch) {
      ...
      if (start < 1 || end > totalPages || start > end) {
        throw new Error(`Invalid range "${part}". Pages go from 1 to ${totalPages}.`);
      }
      for (let i = start; i <= end; i++) indices.add(i - 1);
    } else {
      const num = parseInt(part, 10);
      if (isNaN(num) || num < 1 || num > totalPages) {
        throw new Error(`Invalid page "${part}". Pages go from 1 to ${totalPages}.`);
      }
      indices.add(num - 1);
    }
  }
    ...
    <div className="rounded-2xl border border-border bg-surface p-3 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_-12px_rgba(0,0,0,0.12)] sm:p-4">
      {!file ? (
        ...
        >
          ...
          <p className="mt-1 text-sm text-ink-2">or choose a file to split</p>
          <button
            type="button"
        ...
        <div className="space-y-3">
          ...
          {/* Range input */}
          <div>
            <label htmlFor="page-range" className="block text-sm font-medium text-ink mb-1.5">
              Pages to extract
            </label>
            <input
              id="page-range"
            ...
            <p className="mt-1.5 text-xs text-ink-3">
              Use commas and ranges. Example: 1,3,5-8
            </p>
          </div>
          ...
          {done && (
            <div className="flex items-center gap-3 rounded-lg bg-ok-soft px-3 py-2">
              <p className="text-xs text-ok">Split PDF downloaded successfully!</p>
            </div>
          )}
